...
class Client:
    ...
    Args:
        ...
        http_client: Optional pre-configured ``httpx.Client`` (advanced; you own
            its lifecycle, headers, and transport).
    ...
    def request(self, method: str, path: str, params: dict[str, Any] | None = None) -> Any:
        ...
        for attempt in range(attempts):
            try:
                response = self._http.request(method, path, params=cleaned)
            except httpx.HTTPError as exc:
                if attempt + 1 < attempts:
                    time.sleep(compute_backoff(attempt, self._config.backoff_factor))
                    continue
            ...
            if should_retry(response.status_code) and attempt + 1 < attempts:
                ...
                time.sleep(compute_backoff(attempt, self._config.backoff_factor, retry_after))
                continue
